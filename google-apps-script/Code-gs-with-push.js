  ...
  try {
    ...
    switch(action) {
      ...
      case 'savePushSubscription':
        return createResponse(handleSavePushSubscription(e));
...
function handleMarkCompleted(params) {
  try {
    ...
    // שליחת התראת Push למבקש
    try {
      var row = sheet.getRange(rowIndex, 1, 1, sheet.getLastColumn()).getValues()[0];
      var taskDesc = row[colMap['תיאור המשימה'] - 1] || '';
      var reqEmail = row[colMap['אימייל המבקש'] - 1] || '';
      sendPushToUser(reqEmail, 'המשימה בוצעה ✓', 'המשימה "' + taskDesc.substring(0, 60) + '" סומנה כבוצעה', '/');
    } catch(pushErr) { Logger.log('Push error in markCompleted: ' + pushErr.toString()); }
...
function handleReturnTask(params) {
  try {
    ...
    // שליחת התראת Push למבקש
    try {
      var pushEmail = sheet.getRange(rowIndex, colMap['אימייל המבקש']).getValue() || '';
      var pushDesc = sheet.getRange(rowIndex, colMap['תיאור המשימה']).getValue() || '';
      sendPushToUser(pushEmail, 'משימה הוחזרה להשלמה', 'המשימה "' + pushDesc.substring(0, 60) + '" הוחזרה: ' + reason.substring(0, 100), '/');
    } catch(pushErr) { Logger.log('Push error in returnTask: ' + pushErr.toString()); }
...
/**
 * בדיקת משימות באיחור ושליחת התראות
 * הגדר כ-trigger יומי: Edit > Triggers > Add Trigger
 * Function: checkOverdueTasks, Time-driven, Day timer, 8am-9am
 */
function checkOverdueTasks() {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var tasksSheet = ss.getSheetByName(CONFIG.sheets.tasks);
    if (!tasksSheet) return;

    var data = tasksSheet.getDataRange().getValues();
    var colMap = getColumnMap(tasksSheet);
    ...
    for (var i = 1; i < data.length; i++) {
      var row = data[i];
      var status = row[colMap['סטטוס'] - 1] || '';
      var dueDateValue = row[colMap['תאריך לביצוע'] - 1] || '';
      var email = row[colMap['אימייל המבקש'] - 1] || '';
      var description = row[colMap['תיאור המשימה'] - 1] || '';

      // Skip completed/cancelled
      if (status === 'בוצע' || status === 'בוטל' || status === 'פג תוקף - לא רלוונטי') continue;

      // Check if overdue
      if (dueDateValue) {
        var due = new Date(dueDateValue);
